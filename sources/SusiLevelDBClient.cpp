...
Susi::LevelDBComponent::LevelDBComponent(Susi::SusiClient &susi,
                                         std::string dbPath)
    : susi_{susi}, dbPath_{dbPath} {
  ...
  susi_.registerProcessor(
      "(leveldb|state)::delete",
      [this](Susi::EventPtr event) { handleDelete(std::move(event)); });
...
void Susi::LevelDBComponent::join() { susi_.join(); }

bool Susi::LevelDBComponent::validateFieldsForPut(const Susi::EventPtr &event) {
  if (!(event->payload.isObject()) || !(event->payload["key"].isString()) ||
...
bool Susi::LevelDBComponent::validateFieldsForGetRange(
    const Susi::EventPtr &event) {
  if (!(event->payload.isObject()) || !(event->payload["begin"].isString()) ||
...
void Susi::LevelDBComponent::handlePut(Susi::EventPtr event) {
  ...
  leveldb::Status s =
      db_.get()->Put(leveldb::WriteOptions(), event->payload["key"].getString(),
                     event->payload["value"].toJSON());
  if (!s.ok()) {
    event->headers.push_back({"Error", "leveldb put error: " + s.ToString()});
    return;
  }
  std::cout << "wrote " << event->payload["key"].getString() << " to disk."
            << std::endl;
...
void Susi::LevelDBComponent::handleGet(Susi::EventPtr event) {
  ...
  std::string valueStr;
  leveldb::Status s = db_.get()->Get(
      leveldb::ReadOptions(), event->payload["key"].getString(), &valueStr);
  if (!s.ok()) {
    event->headers.push_back({"Error", "leveldb get error: " + s.ToString()});
    return;
  }
  std::cout << "got " << event->payload["key"].getString() << " from disk."
            << std::endl;
...
void Susi::LevelDBComponent::handleDelete(Susi::EventPtr event) {
  ...
  leveldb::Status s = db_.get()->Delete(leveldb::WriteOptions(),
                                        event->payload["key"].getString());
  if (!s.ok()) {
    event->headers.push_back(
        {"Error", "leveldb delete error: " + s.ToString()});
    return;
  }
  std::cout << "deleted " << event->payload["key"].getString() << " from disk."
            << std::endl;
  event->payload["success"] = true;
}
...
void Susi::LevelDBComponent::handleGetPrefix(Susi::EventPtr event) {
  ...
  for (dbIter->Seek(prefix);
       dbIter->Valid() && dbIter->key().starts_with(prefix); dbIter->Next()) {
  ...
  std::cout << "got all keys with prefix " << prefix << " from disk."
            << std::endl;
...
void Susi::LevelDBComponent::handleGetRange(Susi::EventPtr event) {
  ...
  std::shared_ptr<leveldb::Iterator> dbIter(
      db_->NewIterator(leveldb::ReadOptions()));
  for (dbIter->Seek(begin); dbIter->Valid() && dbIter->key().compare(end) <= 0;
       dbIter->Next()) {
    if (!dbIter->value().empty()) {
      auto key = dbIter->key();
      auto value = dbIter->value();
  ...
  std::cout << "got all keys in range [" << begin << ", " << end
            << "] from disk." << std::endl;
  event->payload["value"] = result;
}
